...
ALTER TABLE [Customers] ADD [LastPurchaseAt] datetimeoffset NULL;

ALTER TABLE [Customers] ADD [LifetimeValue] decimal(18,2) NOT NULL DEFAULT 0.0;

ALTER TABLE [Customers] ADD [PurchaseCount] int NOT NULL DEFAULT 0;
...
CREATE TABLE [LoyaltyCoupons] (
    ...
    CONSTRAINT [PK_LoyaltyCoupons] PRIMARY KEY ([Id])
);

CREATE INDEX [IX_Customers_TenantId_Segment] ON [Customers] ([TenantId], [Segment]);

CREATE INDEX [IX_CustomerPurchases_TenantId_CustomerId_PurchasedAt] ON [CustomerPurchases] ([TenantId], [CustomerId], [PurchasedAt]);

CREATE UNIQUE INDEX [IX_LoyaltyCoupons_TenantId_Code] ON [LoyaltyCoupons] ([TenantId], [Code]);

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20260714231037_V20260714_TenantSecurityCrmLoyalty', N'9.0.0');
